...
/*********************** Print weight update information **********************/
			if (step > 2) {
				...
				if (verbose) {
					if (!extended)
						printf("step %li - lrate %5f, wchange %7.6f, angledelta %4.1f deg\n",step,lrate,change,DEGCONST*angledelta);
					else {
						for (i=0,j=0 ; i<chans ; i++) j += signs[i];
						printf("step %li - lrate %5f, wchange %7.6f, angledelta %4.1f deg, %li subgauss\n",step,lrate,change,DEGCONST*angledelta,j);
					}
				}
			}
			else
				if (verbose) {
					if (!extended)
						printf("step %li - lrate %5f, wchange %7.6f\n",step,lrate,change);
					else {
						for (i=0,j=0 ; i<chans ; i++) j += signs[i];
						printf("step %li - lrate %5f, wchange %7.6f, %li subgauss\n",step,lrate,change,j);
					}
				}
		}
